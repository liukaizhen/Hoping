...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean logRequest = level == Level.ALL || (level != Level.NONE && level == Level.REQUEST);
        if (logRequest) {
            //打印请求信息
        ...
        boolean logResponse = level == Level.ALL || (level != Level.NONE && level == Level.RESPONSE);
        ...
        if (mHandler != null)//这里可以比客户端提前一步拿到服务器返回的结果,可以做一些操作,比如token超时,重新获取
            return mHandler.onHttpResponseBefore(bodyString, chain, originalResponse);
